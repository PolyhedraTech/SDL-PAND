...
confirmed <- infective['values'] + recovered['values'] + deaths['values']
confirmed <- confirmed[seq(1, dim(confirmed)[1], 10),]

casosModel <- diff(confirmed)
reporting = 1 #% dels casos reals
lag = 10 #desfasament
casosModel <- casosModel[lag:length(casosModel)]*reporting/100

# currentModel="Pandemiav2.9h-CAT-Omni"
# currentModel="Pandemiav2.10"
