...
write.csv(casosDiaris,"./SDLModel/casosDiaris.csv",row.names = FALSE)
  ...
  # HospitalitzatsAdults <- Hospitalitzats %>% filter(grup_edat == 'Entre 15 i 64')  
  # HospitalitzatsAdults <- aggregate(HospitalitzatsAdults$ingressos_total, by=list(HospitalitzatsAdults$data), sum)
  # HospitalitzatsAdults$Group.1 <- as.Date(HospitalitzatsAdults$Group.1 , format = "%y/%m/%d")
  # 
  # 
  # HospitalitzatsGrans <- Hospitalitzats %>% filter(grup_edat == 'Entre 65 i 74')  
  # HospitalitzatsGrans <- aggregate(HospitalitzatsGrans$ingressos_total, by=list(HospitalitzatsGrans$data), sum)
  # HospitalitzatsGrans$Group.1 <- as.Date(HospitalitzatsGrans$Group.1 , format = "%y/%m/%d")
  # 
  # HospitalitzatsAvis <- Hospitalitzats %>% filter(grup_edat == 'Majors de 74')  
  # HospitalitzatsAvis <- aggregate(HospitalitzatsAvis$ingressos_total, by=list(HospitalitzatsAvis$data), sum)
  # HospitalitzatsAvis$Group.1 <- as.Date(HospitalitzatsAvis$Group.1 , format = "%y/%m/%d")
  ...
  # if(nrow(VacunacionsDosi1Nens)>0)
  # {
  #   VacunacionsDosi1Nens <- aggregate(VacunacionsDosi1Nens$recompte, by=list(VacunacionsDosi1Nens$data), sum)
  #   VacunacionsDosi1Nens$Group.1 <- as.Date(VacunacionsDosi1Nens$Group.1 , format = "%y/%m/%d")
  # } else
  # {
  #   VacunacionsDosi1Nens=data.frame(Group.1=VacunacionsDosi1All$Group.1,x=c(rep(0, nrow(VacunacionsDosi1All))))
  # }
  # 
  # VacunacionsDosi1Nens$Group.1 <- as.Date(VacunacionsDosi1Nens$Group.1 , format = "%y/%m/%d")
  # VacunacionsDosi1Adults <- aggregate(VacunacionsDosi1Adults$recompte, by=list(VacunacionsDosi1Adults$data), sum)
  # VacunacionsDosi1Adults$Group.1 <- as.Date(VacunacionsDosi1Adults$Group.1 , format = "%y/%m/%d")
  # VacunacionsDosi1Grans <- aggregate(VacunacionsDosi1Grans$recompte, by=list(VacunacionsDosi1Grans$data), sum)
  # VacunacionsDosi1Grans$Group.1 <- as.Date(VacunacionsDosi1Grans$Group.1 , format = "%y/%m/%d")
  # VacunacionsDosi1Avis <- aggregate(VacunacionsDosi1Avis$recompte, by=list(VacunacionsDosi1Avis$data), sum)
  # VacunacionsDosi1Avis$Group.1 <- as.Date(VacunacionsDosi1Avis$Group.1 , format = "%y/%m/%d")

  # VacunacionsDosi2Nens <- VacunacionsDosi2 %>% filter(edat == 'Menors de 15')
  # VacunacionsDosi2Adults <- VacunacionsDosi2 %>% filter(edat == '15 a 64')  
  # VacunacionsDosi2Grans <- VacunacionsDosi2 %>% filter(edat == '65 a 74')  
  # VacunacionsDosi2Avis <- VacunacionsDosi2 %>% filter(edat == '75 o més')  
  ...
  # if(nrow(VacunacionsDosi2Nens)>0)
  # {
  #   VacunacionsDosi2Nens <- aggregate(VacunacionsDosi2Nens$recompte, by=list(VacunacionsDosi2Nens$data), sum)
  #   VacunacionsDosi2Nens$Group.1 <- as.Date(VacunacionsDosi2Nens$Group.1 , format = "%y/%m/%d")
  # } else
  # {
  #   VacunacionsDosi2Nens=data.frame(Group.1=VacunacionsDosi2All$Group.1,x=c(rep(0, nrow(VacunacionsDosi2All))))
  # }
  # 
  # VacunacionsDosi2Nens$Group.1 <- as.Date(VacunacionsDosi2Nens$Group.1 , format = "%y/%m/%d")
  # VacunacionsDosi2Adults <- aggregate(VacunacionsDosi2Adults$recompte, by=list(VacunacionsDosi2Adults$data), sum)
  # VacunacionsDosi2Adults$Group.1 <- as.Date(VacunacionsDosi2Adults$Group.1 , format = "%y/%m/%d")
  # VacunacionsDosi2Grans <- aggregate(VacunacionsDosi2Grans$recompte, by=list(VacunacionsDosi2Grans$data), sum)
  # VacunacionsDosi2Grans$Group.1 <- as.Date(VacunacionsDosi2Grans$Group.1 , format = "%y/%m/%d")
  # VacunacionsDosi2Avis <- aggregate(VacunacionsDosi2Avis$recompte, by=list(VacunacionsDosi2Avis$data), sum)
  # VacunacionsDosi2Avis$Group.1 <- as.Date(VacunacionsDosi2Avis$Group.1 , format = "%y/%m/%d")
...
The aim of the models is to show a pessimistic scenario with a maximum number of cases, this is the reason why the forecasts move in the upper threshold of the detected case curve. To access the indicators (KPIs) and the detailed forecast for the whole of Catalonia, access http://pand.sdlps.com/#tot-catalunya via the top tab. An attempt to describe the drivers of the spread in Catalonia can be reviewed at http://pand.sdlps.com/Pandemic-drivers.html
To see a work in progress scenario you can go to http://pand.sdlps.com/AllCatalunyaAlternative.html
...
file = "../../models/SDL/Pandemiav2.10/regions-ALL/newInfectiveDiary.csv"
...
# [1] L. J. Abu-Raddad et al., “SARS-CoV-2 antibody-positivity protects against reinfection for at least seven months with 95% efficacy,” EClinicalMedicine, vol. 35, p. 100861, May 2021, doi: 10.1016/J.ECLINM.2021.100861/ATTACHMENT/6EF8F234-629B-4BF3-97A4-4374A6AF648F/MMC1.DOCX.
# [2] H. Altarawneh et al., “Protection afforded by prior infection against SARS-CoV-2 reinfection with the Omicron variant,” medRxiv, p. 2022.01.05.22268782, Jan. 2022, doi: 10.1101/2022.01.05.22268782.
# [3] N. Ferguson, A. Ghani, A. Cori, A. Hogan, W. Hinsley, and E. Volz, “Report 49 : Growth, population distribution and immune escape of Omicron in England,” 2021. [Online]. Available: https://www.imperial.ac.uk/media/imperial-college/medicine/mrc-gida/2021-12-16-COVID19-Report-49.pdf.

protectionForReinfectionDelta <- 0.95 # [1]
protectionForReinfectionOmicron <- 0.95 # [2-3] 0.6, 0.19
overCounting <- 0.7 #We detect 30% more cases that the real cases.

# casosReals <- sum(newInfectives2.9$values[datetimes<Sys.Date()])*(1-0.15)*0.8 #15% son reinfeccions i un 20 de variabilitat (movem per la zona superior de casos).
casosTotalsDelta <- sum(newInfectives2.9$values[datetimes<"2021-11-12"])
casosTotalsFinalsOnada <-sum(newInfectives2.9$values[datetimes>="2021-11-12"])
casosTotalsOmicron <- sum(newInfectives2.9$values[datetimes<Sys.Date()])-casosTotalsDelta

casosRealsOmicron <- casosTotalsOmicron*protectionForReinfectionOmicron*overCounting
casosRealsDelta <- casosTotalsDelta*protectionForReinfectionDelta*overCounting 

reinfeccionTotals <- sum(newReinfections2.9$values[])
reinfeccionActuals <- sum(newReinfections2.9$values[datetimes<Sys.Date()])

casosRealsValidacio <- casosRealsDelta+casosRealsOmicron
casosRealsFinalsOnadaValidacio <-casosRealsDelta+(casosTotalsFinalsOnada*protectionForReinfectionOmicron*overCounting)

casosReals <- (sum(newInfectives2.9$values[datetimes<Sys.Date()])-reinfeccionActuals)*overCounting
casosRealsFinalsOnada <-(sum(newInfectives2.9$values[])-reinfeccionTotals)*overCounting

casosPositiusAcumulats <- sum(casosPositius$numcasos)

Positius=casosPositiusAcumulats/PoblacioTotal
Undetected=(casosReals-casosPositiusAcumulats)/PoblacioTotal
Total=1-Positius-Undetected

values <- list(Positius=casosPositiusAcumulats/PoblacioTotal,Undetected=(casosReals-casosPositiusAcumulats)/PoblacioTotal,Total=1-(Positius+Undetected))
...
casosReals <- round(casosReals, digit=0)
valueBox(casosReals, icon = "fa-pencil")
```
...
casosRealsFinalsOnada <- round(casosRealsFinalsOnada, digit=0)
valueBox(casosRealsFinalsOnada, icon = "fa-pencil")
```
...
# #htmltools::includeHTML("http://empty.polyhedra.tech/sdl-pandv1.9/EDAR/map.html")
# #htmltools::includeHTML("EDAR/map.html")
# 
# ICRA_data_DBSS <- ICRA_data[ which(ICRA_data$Codi.EDAR=="DBSS"), ]
# ICRA_data_DBSS$LOG_N1<-log(ICRA_data_DBSS$N1..CG.L.)
# ICRA_data_DBSS$LOG_casos<-log(ICRA_data_DBSS$Positius.diagnosticats.municipis.servits..dia.mostreig)
# 
# ICRA_data_DBSS$LOG_casos[which(!is.finite(ICRA_data_DBSS$LOG_casos))] <- 0
# 
# ICRA_data_DBSS$LOG_casos[!is.finite(ICRA_data_DBSS$LOG_casos)] <- 0
# 
# fig <- plot_ly(type="bar", 
#                x = ICRA_data_DBSS$Data.mostreig,
#                y = ICRA_data_DBSS$LOG_casos,
#                name = 'N1',
#                text = paste(""))
# 
# fig <- fig %>% add_trace(y = ICRA_data_DBSS$LOG_N1, x=ICRA_data_DBSS$Data.mostreig, type='scatter', mode = 'lines', name='casos detectats')
# fig
